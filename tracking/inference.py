        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos, oldProb in self.beliefs.items():
            for newPos, newProb in self.getPositionDistribution(self.setGhostPosition(gameState,oldPos)).items():
                newPositions[newPos] += newProb*oldProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [self.legalPositions[i % len(self.legalPositions)] for i in xrange(self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                ...
                beliefs[p] += emissionModel[dist]
            if beliefs.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(beliefs) for p in self.particles]
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        jail = []
        for index, noisy in enumerate(noisyDistances):
            if noisy is None:
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if i not in jail:
                    prob *= emissionModels[i][util.manhattanDistance(p[i], pacmanPosition)]
            beliefs[p] += prob                 
        if beliefs.totalCount() == 0:
            self.initializeParticles()
        else:
