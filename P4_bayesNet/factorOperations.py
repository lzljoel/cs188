...
def joinFactors(factors):
    ...
    for factor in factors:
        unconditioned = unconditioned + list(factor.unconditionedVariables())
        conditioned = conditioned + list(factor.conditionedVariables())
    ...
    newDict = factors[0].variableDomainsDict()
    newFactor = Factor(unconditioned,conditioned, newDict) 
    for a in newFactor.getAllPossibleAssignmentDicts():
        probability = 1
        for current in factors:
            for currentAssignment in current.getAllPossibleAssignmentDicts():
                if set(currentAssignment.items()).issubset(set(a.items())):
                    probability *= current.getProbability(currentAssignment)
        newFactor.setProbability(a, probability)
    return newFactor    
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for a in newFactor.getAllPossibleAssignmentDicts():
            probability = 0
            for currentAssignment in factor.getAllPossibleAssignmentDicts():
                if set(currentAssignment.items()).issuperset(set(a.items())):
                    probability += factor.getProbability(currentAssignment)
            newFactor.setProbability(a, probability)
        return newFactor    
    return eliminate
